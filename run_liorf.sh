...
liorf() {
    ...
    local launch_type=""
    local bag_files=() # Changed to an array to store multiple bag file paths
    local bag_play_extra_args=() # To capture flags like --start-offset
    local image_tag="latest"
    local docker_image_name="frostlab/liorf_hawii" # Assuming this is your Docker image name
    local parsing_bag_flags=false # New flag to indicate we are now parsing bag play flags
    ...
    if [[ "$1" == "-h" || "$1" == "--help" ]]; then
        ...
        echo "  -d                 Don't play bag files upon starting docker container. Still volumed in."
        ...
        echo "                     Each bag will be mounted into a unique subfolder under '/container_bags'"
        echo "                     in the container and played sequentially."
    ...
    while [[ "$#" -gt 0 ]]; do
        if [[ "${parsing_bag_flags}" == true ]]; then
            # If we're already parsing bag flags, just add all remaining arguments to it
    ...
    if [[ ${#bag_files[@]} -gt 0 ]]; then
        ...
        for host_bag_path_raw in "${bag_files[@]}"; do
            ...
            # Note: /container_bags is created by Docker when the volume is mounted
    ...
    if [[ ${#bag_files[@]} -gt 0 ]]; then
        # Wait for the launch PID and all bag play PIDs
